...
model Condenser
  ...
  SI.SpecificEnthalpy h_in = inStream(port_a.h_outflow) "Specific enthalpy at condenser inlet";
  SI.SpecificEnthalpy h_out = port_b.h_outflow "Specific enthalpy at condenser outlet";
  
  SI.MassFlowRate m_in = port_a.m_flow "Mass flow into the condenser at the inlet";
  SI.MassFlowRate m_out = port_b.m_flow "Mass flow rate into the condenser at the outlet";
  ...
  SI.Power e_destr = e_in - e_out "Specific exergy rate of destruction";
  
  SI.AbsolutePressure p_in = port_a.p "condenser inlet pressure";
  SI.AbsolutePressure p_out = port_b.p "condenser outlet pressure";
  
  SI.HeatFlowRate Q "Heat flow rate into the condenser";
...
equation
  ...
  annotation(
    ...
    Icon(graphics = {Text(origin = {0, 87}, extent = {{-64, -25}, {64, 25}}, textString = "%name"), Rectangle(fillColor = {14, 231, 255}, fillPattern = FillPattern.Solid,lineThickness = 1, extent = {{-80, 60}, {80, -60}})}));
